...
def submit(request, course_id):
    ...
    enrollment = Enrollment.objects.get(user=username, course=course_id)
...
def show_exam_result(request, course_id, submission_id):
    ...
    choices_qty = choices.count()
    correct_qty = 0

    for choice in choices:
        choice_object = Choice.objects.get(id=choice.id)
        if choice_object.is_correct:
            correct_qty += 1

    context['grade'] = int(correct_qty / choices_qty * 100)
